...
use rajac_compiler::{verification_java_classpaths, Compiler, CompilerConfig};
...
fn get_ignored_class_file_mismatches() -> HashSet<&'static str> {
    HashSet::from(["TryCatchFinallyMultipleHandlers.class"])
}
...
fn run() -> RajacResult<()> {
    ...
    let classpaths = verification_java_classpaths();
