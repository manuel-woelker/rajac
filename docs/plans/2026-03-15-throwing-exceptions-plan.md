...
- [ ] Confirm or extend colocated semantic tests for `throw` operand validation.
- [ ] Implement `Stmt::Throw` bytecode emission with `athrow`.
- [ ] Add valid verification fixtures for thrown exceptions in methods.
- [ ] Add valid verification fixtures for thrown exceptions in constructors.
- [ ] Verify abrupt completion after `throw` remains semantically correct.
- [ ] Inspect verification output to confirm `athrow` appears in generated bytecode.
