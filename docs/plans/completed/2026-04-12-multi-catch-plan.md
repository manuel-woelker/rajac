...
This milestone shipped end-to-end multi-catch support across parsing, semantic analysis, bytecode lowering, and verification.

The implementation now:

- parses `|`-separated catch type alternatives into one catch clause
- preserves all catch alternatives in the AST and resolves each one
- validates obviously invalid shapes such as duplicate alternatives and non-Throwable-compatible catch types
- lowers one source multi-catch clause into multiple typed exception-table entries that share one handler body
- reuses the same shared-handler approach for mixed `try`/`catch`/`finally`
- adds parser tests, bytecode tests, valid verification fixtures, and an invalid verification fixture for duplicate alternatives

## What checklist tracks the work?

- [x] Audit the current catch-clause representation across parser, AST, resolution, and bytecode generation.
- [x] Extend the AST catch-clause shape to carry multiple catch types.
- [x] Update the parser to accept `|`-separated catch type alternatives.
- [x] Update resolution to resolve every alternative catch type.
- [x] Add conservative semantic checks for obviously invalid multi-catch alternatives.
- [x] Refactor bytecode catch-entry generation so one catch clause can emit multiple typed entries that share one handler.
- [x] Ensure mixed `try`/`catch`/`finally` lowering reuses the same multi-catch handler-entry generation.
- [x] Add colocated tests for multi-catch parsing and bytecode lowering.
- [x] Add verification fixtures for supported multi-catch examples.
