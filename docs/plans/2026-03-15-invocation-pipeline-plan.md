...
- compute owner internal names from resolved method signatures and current class context
- choose invocation opcode from semantic facts (`static`, `private`, `super`, interface owner)
- use descriptor strings built from resolved signatures rather than expression-shape inference
- reuse the same descriptor-aware stack accounting for all call instructions
- keep unsupported invocation cases as explicit compiler errors instead of placeholder indexes
...
7. Update this checklist to reflect completed work and move the plan to `docs/plans/completed` when the invocation work in this plan is finished.
...
- Full support for placeholder bytecode paths such as `new`, `instanceof`, and array creation
  may require follow-up work outside the exact scope of this first invocation-pipeline pass.
...
Verification must be explicit and should cover both local backend behavior and full
compiler output.

- Add or update colocated tests in `crates/bytecode/src/bytecode.rs` and `crates/bytecode/src/classfile.rs`.
- Run `cargo test -p rajac-bytecode`.
...
- [ ] Add backend invocation metadata/helpers for owner, descriptor, and opcode selection.
- [ ] Track current class context during bytecode generation so implicit and `super` calls have a stable owner.
- [ ] Refactor ordinary method-call lowering to use resolved method signatures and modifiers.
- [ ] Refactor constructor/default-constructor/super-constructor lowering to use shared invocation helpers.
- [ ] Add or update colocated invocation tests.
- [ ] Run `cargo test -p rajac-bytecode`.
- [ ] Run `cargo run -p verification --bin verification`.
- [ ] Run `./scripts/check-code.sh`.
